...
@section('content')
    ...
    @endif
    <form action="{{ route('dashboard.blog.store') }}" method="POST" class="grid lg:grid-cols-2 gap-3 p-4 rounded-[4px] border border-[#0d1117]/[0.12]" enctype="multipart/form-data">
        @csrf
        ...
        <div class="lg:col-span-2">
            ...
            <input type="text" id="title" name="title" placeholder="Masukkan judul blog..." class="w-full mt-2 p-3 rounded-black text-[#0d1117] border border-[#0d1117]/[0.12] rounded-[4px]" required>
            @error('title')
            <p class="message-error text-red-600 mt-1 text-[0.875rem]">{{ $message }}</p>
            @enderror
        </div>
        <div class="lg:col-span-2">
            ...
            <textarea rows="6" id="description" name="description" placeholder="Masukkan konten blog..." class="w-full mt-2 p-3 rounded-black text-[#0d1117] border border-[#0d1117]/[0.12] rounded-[4px]" required></textarea>
            @error('description')
            <p class="message-error text-red-600 mt-1 text-[0.875rem]">{{ $message }}</p>
            @enderror
        </div>
