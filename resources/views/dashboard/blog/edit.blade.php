...
@section('content')
    ...
    <form action="{{ route('dashboard.blog.update', $blog) }}" method="POST"
        ...
        <div class="lg:col-span-2">
            ...
            <div class="wrapper mt-2 flex items-end gap-[6px]">
                <img id="image-preview"
                    src="{{ $blog->image_path ? asset('storage/' . $blog->image_path) : 'https://placehold.co/400x400?text=Image+Not+Found' }}"
                    alt="Foto Blog"
                ...
                <label for="image-input"
                    class="cursor-pointer flex items-center gap-2 mt-2 px-4 py-2 bg-[#A91D3A] hover:bg-[#CA3453] text-white rounded-[4px] text-sm font-medium transition border border-[#A91D3A] shadow-sm">
                    <h1>Tambah Foto</h1>
        ...
        <div class="lg:col-span-2">
            ...
            <input type="text" id="title" name="title" placeholder="Masukkan judul blog..."
                value="{{ $blog->title }}"
        ...
        <div class="lg:col-span-2">
            ...
            <textarea rows="6" id="description" name="description" placeholder="Masukkan konten blog..."
                class="w-full mt-2 p-3 rounded-black text-[#0d1117] border border-[#0d1117]/[0.12] rounded-[4px]" required>{{ $blog->description }}</textarea>
            @error('description')
                <p class="message-error text-red-600 mt-1 text-[0.875rem]">{{ $message }}</p>
            @enderror
        </div>
