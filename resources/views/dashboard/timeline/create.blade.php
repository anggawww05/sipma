...
@section('content')
    ...
    <form action="{{ route('dashboard.timeline.store', ['submission' => $submission->id]) }}" method="POST" class="grid lg:grid-cols-2 gap-3 p-4 rounded-[4px] border border-[#0d1117]/[0.12]" enctype="multipart/form-data">
        ...
        <div>
            ...
            <input type="text" id="title" name="title" placeholder="Masukkan judul pengajuan timeline..." class="w-full mt-2 p-3 rounded-black text-[#0d1117] border border-[#0d1117]/[0.12] rounded-[4px]" required>
            @error('title')
        ...
        <div class="lg:col-span-2">
            ...
            <textarea id="description" name="description" placeholder="Masukkan deskripsi pengajuan timeline..." class="w-full mt-2 p-3 rounded-black text-[#0d1117] border border-[#0d1117]/[0.12] rounded-[4px]" required rows="6"></textarea>
            @error('description')
            <p class="message-error text-red-600 mt-1 text-[0.875rem]">{{ $message }}</p>
            @enderror
        </div>
